...
begin
  ...
  begin
    ...
    if SelStart > 3 then Pre := GetText(SelStart - 2, 1)
    else
      Pre := #13;
...
begin
  ...
  if (Key = 8) and (x = Utf16Length(Utf8ToUtf16('•'))) and
    (TP.HeadIndent = BulletSpace) and IsLeftWideString(Lines[y], Utf8ToUtf16('•')) then
    MarkUp(PStart, 1, psPara);

  // For now I'm just going to turn the delete key off at the ends of paragraphs,
  // it's that much trouble.
  if (Key = 46) and ((SelStart = Utf16Length(Text)) or (GetUText(SelStart, 1)[1] = #13))
  then Key := 0;
  // Do better later.
